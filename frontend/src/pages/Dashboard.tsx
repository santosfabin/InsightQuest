...
export default function Dashboard() {
	...
	const processedData = useMemo(() => {
		...
		if (hasRealTargets) {
			...
			predictions.forEach(p => {
				...
				const player = String(
					p.original_data["Código de Acesso"] ?? "N/A"
				);
			...
			if (dataT1.length > 0)
				scatterDataT1 = [{ id: "Target 1", data: dataT1 }];
			if (dataT2.length > 0)
				scatterDataT2 = [{ id: "Target 2", data: dataT2 }];
			if (dataT3.length > 0)
				scatterDataT3 = [{ id: "Target 3", data: dataT3 }];
		}
		...
		const hasTempoVsPerfData =
			hasRoundData &&
			predictions[0].original_data["Cluster_0"] !== undefined;
			...
			predictions.forEach((p, index) => {
				try {
					...
					if (hasTempoVsPerfData) {
						...
						const player = String(
							p.original_data["Código de Acesso"] ??
								`Row_${index + 1}`
						);
						const isCluster0 =
							Number(p.original_data["Cluster_0"] ?? 0) === 1;
					...
					console.error(
						`ERROR inside loop at index ${index}:`,
						error
					);
				}
			});
		...
		if (hasPerformanceEvolutionData) {
			...
			predictions.forEach(p => {
				...
				if (p.original_data["Cluster_0"] !== undefined) {
					cluster =
						Number(p.original_data["Cluster_0"]) === 1 ? 0 : 1;
				}
			...
			if (cluster0Data.count > 0) {
				performanceEvolutionData.push({
					...
					data: [
						{
							x: "Rodada 1",
							y: cluster0Data.r1 / cluster0Data.count
						},
						{
							x: "Rodada 2",
							y: cluster0Data.r2 / cluster0Data.count
						},
						{
							x: "Rodada 3",
							y: cluster0Data.r3 / cluster0Data.count
						},
						{
							x: "Total",
							y: cluster0Data.total / cluster0Data.count
						}
					]
				});
			}
			...
			if (cluster1Data.count > 0) {
				performanceEvolutionData.push({
					...
					data: [
						{
							x: "Rodada 1",
							y: cluster1Data.r1 / cluster1Data.count
						},
						{
							x: "Rodada 2",
							y: cluster1Data.r2 / cluster1Data.count
						},
						{
							x: "Rodada 3",
							y: cluster1Data.r3 / cluster1Data.count
						},
						{
							x: "Total",
							y: cluster1Data.total / cluster1Data.count
						}
					]
				});
			}
	...
	const generateKeyFindings = useMemo(() => {
			...
			if (allPlayersData) {
				...
				if (improvement > 5) {
					...
					findings.push({
						...
						description: `Performance caiu ${Math.abs(
							improvement
						).toFixed(
							1
		...
		if (processedData.colorDistribution.length > 0) {
			...
			const topColorPerc =
				(topColor.value /
					processedData.colorDistribution.reduce(
						(s, c) => s + c.value,
						0
					)) *
				100;
