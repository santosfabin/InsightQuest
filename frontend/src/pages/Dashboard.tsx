...
export default function Dashboard() {
	...
	const [history, setHistory] = useState<StoredAnalysisResults[]>([]);

	useEffect(() => {
		const loadHistory = async () => {
			try {
				const savedAnalyses = await getHistory();
				setHistory(savedAnalyses);
			} catch (error) {
				console.error(
					"Falha ao carregar o histórico do IndexedDB:",
					error
				);
			}
		};
						...
						{history.length === 0 ? (
							<div className="bg-white rounded-3xl shadow-lg p-8">
								<div className="flex flex-col items-center justify-center py-16">
									...
									<p className="text-gray-500 text-center mb-6">
										...
										<br />
										Faça sua primeira análise para ver o
										histórico aqui.
									</p>
									<button
										onClick={() => {
							...
							<div className="space-y-4">
								{history.map(analysis => (
									<div
										key={analysis.id}
										className="bg-white rounded-2xl shadow-lg p-6 flex items-center justify-between hover:shadow-xl transition-shadow cursor-pointer"
										onClick={() => {
											setResults(analysis);
											setActiveTab("dashboard");
											setShowUpload(false);
